<script lang="ts">
	import type { SubjectCard } from "$lib/types/subjectCard";

	export let subject: SubjectCard;
	export let isEditingSubjects: boolean; // Nový prop na kontrolu editovacieho režimu
	export let onDelete: (id: number) => void; // Funkcia na mazanie karty
	export let onEdit: (item: SubjectCard) => void;
</script>
