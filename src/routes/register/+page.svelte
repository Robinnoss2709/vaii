<script lang="ts">
	...
	const registerSchema = z
		.object({
			...
			password: z
				...
				.regex(/[A-Z]/, 'Heslo musí obsahovať aspoň jedno veľké písmeno.') // obsahuje veľké písmeno
				.regex(/[a-z]/, 'Heslo musí obsahovať aspoň jedno malé písmeno.') // obsahuje malé písmeno
				.regex(/[0-9]/, 'Heslo musí obsahovať aspoň jedno číslo.') // obsahuje číslo
				.regex(/[\W_]/, 'Heslo musí obsahovať aspoň jeden špeciálny znak.'), // obsahuje špeciálny znak (neabecedný znak)
	...
	const handleRegister = async (event: Event) => {
		...
		if (!validationResult.success) {
			...
			errorMessage = validationResult.error.issues.map((issue) => issue.message).join(' ');
			return;
		}

		// Volanie Supabase na registráciu používateľa
		...
		if (error) {
			...
			// Po úspešnej registrácii nastavíme premennú na zobrazenie modálneho okna
			successMessage = 'Úspešne ste sa zaregistrovali!';
			showModal = true; // Zobraziť modálne okno
...
<div class="bg-gray-200 min-h-screen">
	<div class="flex flex-col justify-center items-center text-green-400 p-6">
		...
		<form class="flex flex-col gap-4 w-full max-w-sm" on:submit|preventDefault={handleRegister}>
			<label>
				<input
					bind:value={email}
					type="email"
	...
	{#if showModal}
		<div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
			<div class="bg-white p-6 rounded-lg w-96">
				<h2 class="text-xl font-bold text-green-500">Úspešná registrácia!</h2>
				<p class="mt-4 text-gray-700">
					Na váš email bol odoslaný potvrdzovací email. Skontrolujte svoj inbox a potvrďte
					registráciu.
				</p>
				<div class="mt-6 flex justify-end">
					<button class="basic-button hover-bg-green" on:click={closeModal}> Zatvoriť </button>
				</div>
			</div>
		</div>
	{/if}
